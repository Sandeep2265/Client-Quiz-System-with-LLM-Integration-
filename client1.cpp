...
for (int i = 0; i < num_clients; i++) {
    ...
    threads.push_back(thread([client_name]() { 
        ...
        to_print.push_back(client_name + " selected genre: " + genre);
        ...
        while (true) {
            ...
            to_print.push_back(client_name + " " + server_msg);

            if (server_msg.find("QUES") != string::npos) {
                char opt = 'A' + rand() % 4;
                string answer(1, opt);
                send(sock, answer.c_str(), answer.size(), 0);
                to_print.push_back(client_name + " Answered " + answer);
            }
...
void manualClient() {
    ...
    while (true) {
        ...
        string choice;
        cout << "Server sent KEEP_ALIVE. Type 'ALIVE_OK' if you want to be in connection";
        cin >> choice;
        send(sock, choice.c_str(), choice.size(), 0);
        }
    }
